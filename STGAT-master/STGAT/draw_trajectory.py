...
parser.add_argument("--obs_len", default=8, type=int) #調整觀察的frame數
parser.add_argument("--pred_len", default=8, type=int) #調整預測的frame數，圖片上總frame數=觀察frame數+預測frame數 
...
def plot_trajectory(args, loader, generator):
    ...
    pic_cnt = 0
    first_write = True
    predict_frame_num = args.obs_len #開始預測的frame數
    with torch.no_grad():
        for batch in loader:
            ...
            for (start, end) in seq_start_end:
                ...
                print('x',model_output_x_piccoor) ##拿過去8frame的資料預測未來8frame的位置x
                model_output_y_piccoor = (
                    model_output_traj_best[:, start:end, :].cpu().numpy()[:, :, 1].T
                )
                print('y', model_output_y_piccoor) ##拿過去8frame的資料預測未來8frame的位置y
                path = '././datasets/testdata/predict/predict.txt'
                if(first_write):
                    f = open(path, 'w')
                    first_write = False
                else:
                    f = open(path, 'a')
                f.write('Predicted from frame:')
                f.write(str(predict_frame_num+1))
                f.write('\n')
                predict_frame_num += 1
                for id in range(len(model_output_x_piccoor.tolist())):
                    f.write('ID:')
                    f.write(str(id))
                    f.write('\n')
                    f.write(' x:')
                    for x_pos in model_output_x_piccoor.tolist()[id]:
                        f.write(str(round(x_pos,3))) #取小數點後3位
                        f.write(' ')
                    f.write('\n')

                    f.write(' y:')
                    for y_pos in model_output_y_piccoor.tolist()[0]:
                        f.write(str(round(y_pos,3))) #取小數點後3位
                        f.write(' ')
                    f.write('\n')
                print(model_output_y_piccoor.tolist()[0])
                #f.write(model_output_y_piccoor)
                f.close()
...
def main(args):
    ...
    path = get_dset_path('testdata', args.dset_type)#改成要predict的dataset名子
